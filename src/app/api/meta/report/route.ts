...
import { getCampaigns, getCampaignInsights, getAccountInsights } from '@/lib/meta-api';
import type { DateRange, CampaignStatus } from '@/types/meta';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [campaigns, accountInsights] = await Promise.all([
      getCampaigns(statuses, dateRange),
      getAccountInsights(dateRange),
    ]);

    const campaignsWithInsights = await Promise.all(
      campaigns.map(async (c) => {
        const insights = await getCampaignInsights(c.id, dateRange, 'campaign').catch(() => []);
